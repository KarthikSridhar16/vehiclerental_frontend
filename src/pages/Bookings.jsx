...
const holdInfo = (b) => {
  const until =
    b?.pendingHoldUntil ||
    (b?.createdAt ? dayjs(b.createdAt).add(15, "minute").toISOString() : null);
...
useEffect(() => {
    ...
    const idsNeeded = [
      ...new Set(
        rows
          .map((b) => b?.vehicleId)
          .filter(Boolean)
          .filter((id) => !vehiclesById[id] && !bHasVehicleObj(rows, id))
      ),
    ];
  ...
  useEffect(() => {
    ...
    const vids = [
      ...new Set(
        rows.map((b) => b?.vehicleId || b?.vehicle?._id).filter((x) => typeof x === "string" && x)
      ),
    ];
  ...
  const reasonFor = (b) => {
    ...
    if (!dayjs(b?.end).isBefore(now.add(1, "minute")))
      return "You can review after your rental ends";
    return "";
  };
              ...
              rows.map((b) => {
                ...
                return (
                  <tr key={b?._id} className="border-t">
                    <td className="p-3">
                      <span className="inline-block max-w-[240px] truncate" title={label}>
                        {label}
                      </span>
                    </td>
                    <td className="p-3">
                      {dayjs(b?.start).format("D/M/YYYY, h:mm:ss a")} →{" "}
                      {dayjs(b?.end).format("D/M/YYYY, h:mm:ss a")}
                    </td>
                    <td className="p-3">
                      <span
                        className={
                    ...
                    <td className="p-3">
                      {formatMoney(b?.price?.total, b?.price?.currency || "INR")}
                    </td>
                    <td className="p-3">
                      <div className="flex items-center gap-3">
                        <button className="px-3 py-1 border rounded" onClick={() => openDetails(b)}>
                          Details
                        </button>
                        <button
                          className={`px-3 py-1 border rounded ${
                            canWrite(b) ? "hover:bg-gray-50" : "opacity-50 cursor-not-allowed"
                          }`}
        ...
        <div className="bk-modal fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl w-[560px] max-w-[95vw] shadow-lg border border-white/20">
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
              ...
              <button className="text-gray-400 hover:text-gray-600" onClick={closeDetails}>
                ✕
              </button>
            </div>
            <div className="p-4 space-y-3 text-sm">
              ...
              <Row
                ...
                value={
                  <>
                    {dayjs(detailsBooking.start).format("DD MMM YYYY, hh:mm a")}{" "}
                    <span className="opacity-60">→</span>{" "}
                    {dayjs(detailsBooking.end).format("DD MMM YYYY, hh:mm a")}
                  </>
                }
              />
              <Row
                label="Amount"
                value={formatMoney(
                  detailsBooking?.price?.total,
                  detailsBooking?.price?.currency || "INR"
                )}
              />
              ...
              {detailsBooking?.payment?.orderId && (
                <Row
                  label="Order ID"
                  value={<code className="text-xs">{detailsBooking.payment.orderId}</code>}
                />
              )}
              {detailsBooking?.payment?.paymentId && (
                <Row
                  label="Payment ID"
                  value={<code className="text-xs">{detailsBooking.payment.paymentId}</code>}
                />
              )}
            </div>
            <div className="px-4 pb-4 pt-2 flex justify-end">
              <button className="px-4 py-2 border rounded" onClick={closeDetails}>
                Close
              </button>
            </div>
          </div>
    ...
    <div className="rv-card rounded-xl w-[520px] max-w-[95vw] shadow-2xl">
      ...
      <form onSubmit={submitReview} className="rv-body space-y-5">
        ...
        <div className="rv-actions">
          <button type="button" className="btn-ghost" onClick={closeReview} disabled={submitting}>
            Cancel
          </button>
